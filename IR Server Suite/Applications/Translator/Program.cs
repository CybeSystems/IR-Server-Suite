...
namespace Translator
{
    internal static class Program
    {
        ...
        private static readonly string DefaultConfigFile = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Translator\\Translator.xml");

        internal static readonly string FolderDefaultSettings = Path.Combine(AppDomain.CurrentDomain.BaseDirectory,
        ...
        private static LearnStatusDelegate _LearnStatus = null;
        ...
        private static bool _launcherFormVisible;
        ...
        private static void Main(string[] args)
        {
            ...
            IrssLog.Info("Log Verbosity Level: " + Program.Config.LogVerbosity.ToString());
            IrssLog.LogLevel = (IrssLog.Level)Enum.Parse(typeof(IrssLog.Level), Program.Config.LogVerbosity);
        ...
        {
            IrssLog.Info("Show Launcher");

            if (_launcherFormVisible)
            {
                IrssLog.Info("Launcher already visible");
            }
            else
            {
                ThreadStart threadStart = LauncherThread;
                Thread thread = new Thread(threadStart);
                thread.Name = "Translator Launcher";
                thread.IsBackground = true;
                thread.Start();
            }
        }

        private static void LauncherThread()
        {
            try
            {
                _launcherFormVisible = true;

                using (LauncherForm launcherForm = new LauncherForm())
                    launcherForm.ShowDialog();

                _launcherFormVisible = false;
            }
            catch (Exception ex)
            {
                IrssLog.Error(ex);
        ...
        internal static void UpdateNotifyMenu()
        {
            ...
            _notifyIcon.ContextMenuStrip.Items.Add(new ToolStripSeparator());
            _notifyIcon.ContextMenuStrip.Items.Add("&Launcher", null, ClickLauncher);
        ...
        private static void ClickLauncher(object sender, EventArgs e)
        {
            ShowLauncher();
        }
                ...
                switch (received.Type)
                {
                    ...
                    case MessageType.LearnIR:
                        if ((received.Flags & MessageFlags.Success) == MessageFlags.Success)
                        {
                            ...
                            if (_LearnStatus != null) _LearnStatus("Learned IR Successfully", true);
                        }
                        else if ((received.Flags & MessageFlags.Failure) == MessageFlags.Failure)
                        {
                            IrssLog.Error("Failed to learn IR command");
                            if (_LearnStatus != null) _LearnStatus("Failed to learn IR command", false);
                        }
                        else if ((received.Flags & MessageFlags.Timeout) == MessageFlags.Timeout)
                        {
                            IrssLog.Warn("Learn IR command timed-out");
                            if (_LearnStatus != null) _LearnStatus("Learn IR command timed-out", false);
                        }
        ...
        private static void RemoteHandlerCallback(string deviceName, string keyCode)
        {
            if (_inConfiguration)
            {
                if (_mainForm != null) _mainForm.EventAdd(deviceName, keyCode);
                return;
            }
        ...
        public static void BlastIR(string fileName, string port)
        {
            if (!_registered)
        ...
        public static void ProcessCommand(string command, bool async)
        {
            if (async)
            {
                try
        ...
        private static void ProcCommand(object commandObj)
        {
            try
            {
                ...
                {
                    _variables.Save(command.Substring(Common.CmdPrefixSaveVars.Length));
                }
                else if (command.StartsWith(Common.CmdPrefixLoadVars, StringComparison.OrdinalIgnoreCase))
                {
                    _variables.Load(command.Substring(Common.CmdPrefixLoadVars.Length));
                }
